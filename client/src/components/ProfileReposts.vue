<template>
  ...
  <div class="profile-container-top">
    ...
    <div class="post-types">
      ...
      <router-link :to="'/' + username + '/reposts'" class="post-type-active"
        >Reposts</router-link
      >
  ...
  >
    <h2>@{{ username }} hasn't reposted any post.</h2>
  </div>
</template>
  ...
  methods: {
    ...
    async loadProfile(route) {
      try {
        ...
        this.posts = await getReposts(username);
      } catch (e) {
        console.log(e);
      }
    },
    ...
    async postDelete(id) {
      deletePost(id);
      this.posts = await getReposts(this.profileData._id);
    },
